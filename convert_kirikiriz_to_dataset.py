...
def convert_and_save(
    ...
    for script_file_idx, script_file in enumerate(script_files):
        ...
        for dialogue in dialogues:
            ...
            if is_loop_voice:
                positive_prompt.append("sound:loop")

            # 从映射表获取角色特定标签
            character_label = character_label_mapping.get(character_name, {})
...
def parse_args(args: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("-p", "--global-positive-prompt", type=str, action="append", default=[], help="给提取出来的数据集加全局正面标签，可以多次使用此选项。例如: 用`-p source:千恋＊万花`表示数据集来源")
...
def main(args: argparse.Namespace):
    ...
    # 打印转换成功信息
    print(f"Kirikiri Z 游戏提取脚本已转换为 TkTTS 通用数据集，保存在 {args.output_dir}")
